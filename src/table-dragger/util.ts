...
import from = require('array.from');
...
export const sort = (params: {list, from, to, parent?}) => {
    if (params.from < params.to) {
        appendSibling({target: params.list[params.from], origin: params.list[params.to], parent: params.parent});
    } else {
        insertBeforeSibling({target: params.list[params.from], origin: params.list[params.to]});
    }
};
